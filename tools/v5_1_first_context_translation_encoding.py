...
def build_runtime_codec_constraints(
    *,
    ...
    for row_index, context_row in enumerate(context_rows):
        ...
        anchored_record = (
            runtime_records[row_index]
            if row_index < len(runtime_records or [])
            else None
        )
        if row_index == 0 and anchored_record is not None:
            if (
                anchored_record.get("length_offset")
                != mapped_record.get("length_offset")
                or anchored_record.get("record_length_bytes")
        ...
        candidates: list[tuple[str, dict[str, object]]] = [
            ("captured-runtime-coordinate", mapped_record)
        ]
        if (
            anchored_record is not None
            and (
                anchored_record.get("length_offset")
                != mapped_record.get("length_offset")
                or anchored_record.get("record_length_bytes")
            ...
            candidates.append(
                ("visible-anchor-consecutive-fallback", anchored_record)
            )
        ...
        if selected is None:
            ...
            raise ValueError("first context runtime record candidates are missing")
    ...
    *,
    ...
    for expected_index, (target_row, visuals, page_spec, constraint) in enumerate(
        ...
        if expected_index == 1 and proven_first_row_page is not None:
            ...
            if (
                not direct_renderer_first_row
                and constraint is not None
                and constraint.get("target_selector") == 2
                and constraint.get("target_ordinal") == 147
            ):
                row["compact_terminal_record"] = True
        if constraint is not None:
            for key in (
                "target_selector",
                "target_ordinal",
    ...
    if direct_renderer_first_row:
        ...
        if capture_path.is_file():
            ...
            if (
                ...
                and capture.get("schema_version") in {5, 6, 7, 8}
