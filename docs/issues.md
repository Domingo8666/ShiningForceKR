...
| SFKR-045 | 번역+종료 비트 접두부만 덮고 원본 꼬리를 보존한 후보도 정상 한글 뒤 깨진 문자가 5/5 화면에서 표시 | fixed-static | 소비 추적은 기대 문맥 16개 중 4개만 관측해 정렬 증거로 쓰지 않음. 대신 기존 디코더 프롤로그의 고정 출력 카운트, 고정 수 무변경 왕복, 화면 추적의 기대 심볼 수를 결합해 종료 바이트가 소비 종료 조건이 아님을 확정. 이후 안전 진단에서 화면 증거가 확정한 첫 레코드는 19심볼인데 번역 인코더가 프로젝션의 다른 9심볼 레코드를 사용한 주소 불일치를 확인했다. 문맥 프로젝션은 번역 검토에만 사용하고, 바이너리 재삽입은 검증된 화면 물리 주소와 연속 길이 접두 레코드 5개를 직접 따라가도록 수정했다. 승인 문장은 그대로 보존하며 화면용 ASCII 공백·끝 느낌표 삭제는 한글 음절·명령형 말투를 보존하는 범위에서만 허용한다. `encode/decode_symbol_count` 전수 왕복을 통과한 단일 후보만 다음 실행 검증에 사용 |
