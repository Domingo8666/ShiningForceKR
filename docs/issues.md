...
| SFKR-028 | 무입력·버튼 경로 매트릭스 첫 실행이 세부 분류 없는 `runtime-command` 종료 코드 1로 끝남 | fixed-static | renderer probe도 경로 없는 schema v3 실패 영수증을 기록해 실패 단계·종류·마지막 MCP 메서드를 다음 자동실행에서 구분 |
