...
| SFKR-036 | 픽셀 변화가 난 0x0204B1 stream을 상위 디코더 블록 엔트리와 연결하지 못함 | investigating | target read 시점의 DE는 임시 상태이므로 0x33FA 디코더 진입 DE를 별도 캡처하고, 정적 선택자 2가 확인된 호출 뒤 read만 포인터 테이블 엔트리로 확정 |
