...
| SFKR-045 | 번역+종료 비트 접두부만 덮고 원본 꼬리를 보존한 후보도 정상 한글 뒤 깨진 문자가 5/5 화면에서 표시 | investigating | 패딩·꼬리 변형 실험을 중단하고 확정 대사 첫 행에서 종료 심벌 뒤 Huffman 문맥 read가 발생하는지 직접 추적해 소비 루프의 종료 계약을 확정 |
