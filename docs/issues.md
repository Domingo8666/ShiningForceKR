...
| SFKR-045 | 번역+종료 비트 접두부만 덮고 원본 꼬리를 보존한 후보도 정상 한글 뒤 깨진 문자가 5/5 화면에서 표시 | fixed-static | 소비 추적은 기대 문맥 16개 중 4개만 관측해 정렬 증거로 쓰지 않음. 대신 기존 디코더 프롤로그의 고정 출력 카운트, 고정 수 무변경 왕복, 화면 추적의 기대 심볼 수를 결합해 종료 바이트가 소비 종료 조건이 아님을 확정. 19심볼 첫 행은 승인된 12글자 형태로 가능한 단일 페이지·두 페이지 제어 경로를 모두 탐색했으나 도달 불가였으므로, 승인 문장은 그대로 보존하고 화면용에서 ASCII 공백과 끝 느낌표만 제거해 한글 음절·명령형 말투를 보존한 9글자/페이지 제어 3회 형태로 검증한다. `encode/decode_symbol_count` 전수 왕복을 통과한 단일 후보만 다음 실행 검증에 사용 |
