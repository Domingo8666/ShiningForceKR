...
| SFKR-005 | 부자연스럽거나 문맥에 맞지 않는 번역 | open | 원문 확정 뒤 엔트리별 사람 검수 + `hancharacter` `translator_declared` 말투 계약 |
...
| SFKR-036 | DE=2·B=147을 곧바로 148번째 선택 대사의 서수로 가정 | fixed-emulator | 추정한 0x49E0·0x49E1 read가 같은 화면 경로에서 관측되지 않았고 23번째 엔트리 시험도 목표 표식을 표시하지 않아 두 서수 해석을 기각 |
...
| SFKR-038 | 선택 그룹이 확정돼도 기존 stream 준비 판정 하나가 실패하면 시험 빌드 루프를 성공 코드로 조용히 건너뜀 | fixed-static | 검증된 baseline SHA-256·schema v5 그룹·무변경 prefix 왕복을 별도 준비 게이트로 추가하고 stream 또는 group 하나가 준비되면 실제 선택 엔트리 빌드·캡처를 강제 |
| SFKR-039 | 31비트 시험 문구가 106비트 엔트리에서 일찍 종료돼 뒤 엔트리 해석에 쓰일 수 있는 원래 꼬리를 남김 | fixed-static | 화면에 픽셀을 그리지 않는 페이지 선택 제어만 사용해 `한다`와 종료 심벌을 정확히 106비트로 인코딩하고 마지막 페이지도 6으로 복원한 뒤 전체 Huffman 왕복·회귀 통과 |
| SFKR-040 | 새 target read가 관측되지 않은 실행의 S25U 로컬 상세 보고서 기록 실패가 안전 캡처 게시까지 중단 | fixed-static | 로컬 상세 JSON 기록은 최선 노력으로 격리하고, target read 미확정 시 선택자·그룹 증거를 안전 캡처에 결합하지 않은 채 검증 가능한 실패 상태를 계속 게시 |
| SFKR-042 | 최신 화면 캡처가 실패 상태로 갱신되면 앞서 확정한 그룹 경계가 사라지고 이전 시험 ROM·보고서를 재사용할 수 있음 | fixed-static | 확정 그룹 캡처를 `analysis/evidence/`에 불변 입력으로 고정하고, 매 시험 전에 로컬 시험 산출물을 제거한 뒤 새 ROM과 보고서가 실제 생성되지 않으면 화면 캡처 전에 실패시킴 |
| SFKR-041 | B=147 후보와 0x44B1 포함 그룹 후보가 실제 표시 스트림이라는 직접 증거 없음 | fixed-emulator | 23번째 그룹 엔트리의 200비트 전체를 바꾼 시험 ROM(e263a1ec…)에서도 0x44B1 read가 재현되지 않아 그룹 해석을 표시 엔트리에서 제외. 런타임에서 직접 시작점으로 관측되고 픽셀 변화가 확인된 0x44B1의 87비트 스트림을 정확한 길이로 재시험 |
| SFKR-043 | 디코더 진입의 B=147 의미와 0x43DE.. 런타임 read 묶음의 종료 조건이 미확정 | fixed-emulator | ROM 바이트 없는 64명령 추적에서 `INC B; ...; DJNZ`와 `E=(HL); HL++; HL+=DE` 반복을 확인. B=147은 길이 접두 레코드 147개를 건너뛰는 횟수이며 0x44B1은 중간 길이 바이트로 확정 |
| SFKR-044 | 원본 레코드 길이에 맞춘 제어 토큰 채움 뒤 번역문 다음에 깨진 문자가 5/5 화면에서 표시 | fixed-static | 종료 전 채움 가설을 기각하고 번역+종료 비트 접두부만 덮은 뒤 나머지 원본 비트를 보존하도록 변경. 정적 비트 보존을 전수 검사한 단일 후보만 실행 검증하며, 깨짐이 남으면 추가 패딩 실험 없이 소비 루프 종료 추적으로 전환 |
| SFKR-045 | 번역+종료 비트 접두부만 덮고 원본 꼬리를 보존한 후보도 정상 한글 뒤 깨진 문자가 5/5 화면에서 표시 | fixed-static | 소비 추적은 기대 문맥 16개 중 4개만 관측해 정렬 증거로 쓰지 않음. 대신 기존 디코더 프롤로그의 고정 출력 카운트, 고정 수 무변경 왕복, 화면 추적의 기대 심볼 수를 결합해 종료 바이트가 소비 종료 조건이 아님을 확정. 이후 안전 진단에서 화면 증거가 확정한 첫 레코드는 19심볼인데 번역 인코더가 9심볼만 복원함을 확인했다. 추가 진단으로 서로 같은 원문 행에 연결된 복수 타깃 중 마지막 항목이 덮어쓰이는 인덱스 결함을 확인했다. 인코딩과 재삽입 모두 원문 행 번호 대신 각 화면에서 직접 관측한 selector/ordinal을 유일 키로 사용하고, 첫 화면의 검증된 물리 주소와 일치하는지 확인하며, 선택한 길이·주소를 로컬 인코딩 결과에서 실제 ROM 쓰기 단계까지 동일성 검사한다. 첫 화면은 19심벌 무변경 왕복이 입증된 `CANDIDATE_END_SYMBOL`을 강제하고, 후속 화면도 이를 우선한다. 좌표 주소와 앵커 연속 체인이 모두 이 문맥에서 실패한 행만 실제 벡터 관측 문맥을 사용하되 원본 비트 무변경 왕복을 다시 요구한다. 승인 문장은 그대로 보존하며 화면용 ASCII 공백·끝 느낌표 삭제는 한글 음절·명령형 말투를 보존하는 범위에서만 허용한다. `encode/decode_symbol_count` 전수 왕복을 통과한 단일 후보만 다음 실행 검증에 사용 |
| SFKR-046 | 실제 레코드 5개가 주소·문맥·길이 검사를 통과한 뒤 고정 수 폰트 경로 탐색이 실패 행 없이 중단 | fixed-static | 후보를 전체 244페이지로 무제한 확장하지 않고 기존 Huffman 도달성 순위 상위 8페이지까지 탐색하며, 선택 단계에서도 행 번호와 단일/다중 페이지 실패 종류를 안전 영수증에 기록 |
...
- 화자별 원문/번역 행이 만들어지면 `hancharacter`로 말투 밀도 LOSS와
  INVENTION을 검사합니다. 현재 canonical 유형이 없으므로
  `precedent_backed`가 아니라 `translator_declared` 권위만 사용하며,
  기계 검사는 사람의 문맥 승인과 의미 충실도 판단을 대체하지 않습니다.
