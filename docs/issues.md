...
| SFKR-047 | 새 ROM에서 예정한 9개 Huffman 문맥은 전부 일치했지만 종료 심벌 문맥으로 한 번 더 조회했고, 화면에는 인라인 페이지 선택 3심벌까지 글자로 표시 | fixed-static | 이 대사 소비자는 종료 심벌 정지가 아니라 10심벌 고정 소비이며 인라인 `0x5F` 페이지 선택을 해석하지 않는 것으로 판정. 첫 행 기술 PoC는 인라인 페이지 선택을 제거하고, 정적 글리프 대조로 동률인 암시 페이지 후보 최대 16개에 서로 다른 글리프 슬롯 9개를 동일하게 배치하며 마지막 종료 심벌 1개까지 최대 128비트 안에서 공동 탐색한다. `두고봐라!` 뒤 남는 표시 슬롯은 검증 가능한 빈 타일로 채운 단일 후보만 재캡처 |
