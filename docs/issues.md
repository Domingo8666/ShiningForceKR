...
| SFKR-045 | 번역+종료 비트 접두부만 덮고 원본 꼬리를 보존한 후보도 정상 한글 뒤 깨진 문자가 5/5 화면에서 표시 | fixed-static | 소비 추적은 기대 문맥 16개 중 4개만 관측해 정렬 증거로 쓰지 않음. 대신 기존 디코더 프롤로그의 고정 출력 카운트, 고정 수 무변경 왕복, 화면 추적의 기대 심볼 수를 결합해 종료 바이트가 소비 종료 조건이 아님을 확정. 이후 안전 진단에서 화면 증거가 확정한 첫 레코드는 19심볼인데 번역 인코더가 9심볼만 복원함을 확인했다. 추가 진단으로 서로 같은 원문 행에 연결된 복수 타깃 중 마지막 항목이 덮어쓰이는 인덱스 결함을 확인했다. 인코딩과 재삽입 모두 원문 행 번호 대신 각 화면에서 직접 관측한 selector/ordinal을 유일 키로 사용하고, 첫 화면의 검증된 물리 주소와 일치하는지 확인하며, 선택한 길이·주소를 로컬 인코딩 결과에서 실제 ROM 쓰기 단계까지 동일성 검사한다. 각 화면에서 첫 Huffman 벡터 read로 직접 포착한 초기 문맥을 사용하며, 좌표 프로젝션 레코드가 무변경 왕복에 실패하면 화면 앵커에서 검증된 연속 길이 접두 체인을 같은 실행에서 판정한다. 승인 문장은 그대로 보존하며 화면용 ASCII 공백·끝 느낌표 삭제는 한글 음절·명령형 말투를 보존하는 범위에서만 허용한다. `encode/decode_symbol_count` 전수 왕복을 통과한 단일 후보만 다음 실행 검증에 사용 |
