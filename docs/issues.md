...
| SFKR-043 | 디코더 진입의 B=147 의미와 0x43DE.. 런타임 read 묶음의 종료 조건이 미확정 | investigating | 패치 코드의 `INC B`를 B+1=148개 고정 출력 심벌 가설로 격리하고, 무변경 고정-count 디코드↔인코드·0x44B1 포함·다음 포인터 범위를 모두 통과할 때만 출력 길이를 보존한 다중 `한다` 기술 표식 시험을 생성 |
