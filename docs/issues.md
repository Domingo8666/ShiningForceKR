...
| SFKR-040 | 실제 선택 엔트리의 첫 바이트가 직전 엔트리와 비트 단위로 공유돼 새 목표 read 캡처가 안전 산출물을 만들지 못함 | fixed-static | 선택 엔트리 안쪽 두 번째 바이트를 화면 breakpoint로 사용하고 패치 쓰기 시작은 원래 bit 7에 유지하며, 목표 read 미확정 시 선택자 증거를 안전 캡처에 결합하지 않도록 수정 |
