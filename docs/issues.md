...
| SFKR-041 | 최신 화면 캡처가 실패 상태로 갱신되면 앞서 확정한 그룹 경계가 사라지고 이전 시험 ROM·보고서를 재사용할 수 있음 | fixed-static | 확정 그룹 캡처를 `analysis/evidence/`에 불변 입력으로 고정하고, 매 시험 전에 로컬 시험 산출물을 제거한 뒤 새 ROM과 보고서가 실제 생성되지 않으면 화면 캡처 전에 실패시킴 |
| SFKR-041 | B=147 후보는 무변경 왕복만 확인됐고 실제 화면 read 0x44B1이 속한 엔트리와 다름 | fixed-static | schema v5의 유일한 target-byte 후보인 23번째 엔트리(bit 1550..1750, 200비트)를 직접 선택하고, 200비트 길이 보존 `한다`와 원래 관측 좌표 0x44B1 breakpoint로 재검증 |
