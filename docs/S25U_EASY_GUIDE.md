...
## 원문-한글 연결 검토를 요청받았을 때

1. **내 파일** 앱에서
   `내부 저장공간 > ShiningForceKR > reports > HUMAN_REVIEW`로 이동합니다.
2. `SOURCE_TARGET_SECTION_REVIEW_LATEST.txt`를 먼저 엽니다.
3. 그 안의 `열 파일:` 뒤에 적힌 검토표를 엽니다.
4. 각 항목에서 `승인`, `거부`, `보류` 중 하나의 `[ ]`만 `[x]`로 바꿉니다.
5. 한글 문장까지 직접 승인할 때만 `승인 한글문:` 뒤에 적습니다.
6. 저장한 뒤 Codex에 **“검토표 저장했어”**라고 말합니다.

원문, 현재 한글, 화자, 대상 번호는 수정하지 않습니다. 고정 ID가 붙은
검토표는 자동실행기가 덮어쓰지 않으므로 한 번에 전부 작성하지 않아도 됩니다.
