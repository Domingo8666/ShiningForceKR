...
- 화면에서 확인된 정확한 길이 접두 엔트리의 경계·왕복·표식 증거
- 확정 레코드 직후 한 프레임의 VDP 출력·디코더 후보 히트 집계
...
진행 사진 요청에 따라 시험 화면 중 표식 검출에 사용한 가장 늦은 프레임 1장만
...
않습니다. 새 시험 빌드에서 아직 비교 파일이 없으면 진행 PNG를 먼저
안전 커밋한 뒤, 같은 런타임 단계가 기준판 콜드 부팅과 픽셀 비교를
즉시 이어서 실행합니다.
...
사람 검토가 시험 문구 표시를 통과하면
`tools/v5_1_visible_entry_proof.py`가 S25U 로컬 빌드 보고서의
길이 접두 엔트리와 캡처·비교·검토 해시를 다시 대조합니다. 모두 같은
빌드와 물리·논리 좌표를 가리킬 때만
`v5_1_latest_visible_entry_proof.json`에 엔트리 경계, 심벌·비트 수와
왕복 판정을 게시합니다. 원문 심벌과 ROM 바이트는 포함하지 않으며,
이 증거는 다음 다중 글자 PoC를 열지만 전체 번역 빌드를 허용하지는
않습니다.

다중 글자 화면에서는 `tools/v5_1_poc_expansion_proof.py`가 최신 진행
PNG에서 `시험한다` 네 글자의 연속 8×8 잉크 마스크를 찾습니다. 정확히
한 위치에서 일치하고, 기준판 비교·사람 검토·100비트 왕복과 빌드 해시가
...
번호와 디코더 전달 주소를 다시 확인한 뒤 한 프레임의 제한된 trace에서
VDP 데이터·제어 포트 출력을 분류합니다.
