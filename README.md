...
- DE=2에서 포인터 0x43DE를 고르는 디코더 진입과 B=147을 확인했습니다. B를 148번째 문자열 번호, 148개 고정 출력 심벌, 0x44B1 독립 스트림, 23번째 그룹 엔트리로 본 네 해석은 모두 런타임 read 범위 또는 화면 시험에서 기각했습니다. 다음 자동 단계는 ROM 바이트를 게시하지 않는 192명령 레지스터 추적으로 `INC B` 전후의 BC·DE·HL 역할을 확정하고 실제 출력 버퍼와 압축 스트림의 연결을 찾습니다.
