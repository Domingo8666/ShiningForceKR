...
- DE=2·B=147에서 시작 포인터 0x43DE와 148번째 비트패킹 엔트리를 재현했습니다. 런타임에서 읽힌 0x44B1은 그 148번째 엔트리가 아니라 앞선 23번째 엔트리 안에 있습니다. 0x44B1을 독립 87비트 스트림으로 바꾼 시험은 픽셀을 바꿨지만 목표 문구를 확정하지 못했으므로, 선택 대사가 아니라 148번째 엔트리까지 건너뛰는 중간 읽기라는 새 가설을 검사합니다. 다음 기술 시험은 148번째 엔트리 시작 0x49E0의 비트 7에서 정확히 106비트를 보존한 `한다` 표식을 만들고, 그 시작 바이트의 직접 read가 확인될 때만 화면을 캡처합니다. 기존 그룹 분석은 `analysis/evidence/v5_1_confirmed_group_capture.json`에 고정합니다.
