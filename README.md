...
- DE=2에서 시작 포인터 0x43DE와 연속 비트패킹 엔트리를 재현했습니다. 런타임에서 읽힌 0x44B1은 23번째 엔트리 안에 있으며, 그 바이트를 독립 87비트 스트림으로 바꾼 시험은 픽셀을 바꿨지만 목표 문구를 표시하지 못했습니다. B=147을 건너뛰기 번호로 해석해 예측한 148번째 엔트리 시작 0x49E0은 후속 실행에서 전혀 읽히지 않아 자동 기각했습니다. 다음 기술 시험은 실제 read가 유일하게 속한 23번째 엔트리의 시작 비트에서 정확히 200비트를 보존한 `한다` 표식을 만들고, 해당 시작 바이트의 직접 read가 확인될 때만 화면을 캡처합니다. 기존 그룹 분석은 `analysis/evidence/v5_1_confirmed_group_capture.json`에 고정합니다.
