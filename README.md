...
- 공개 영문 대본 25개 구간·1,409줄과 화자 표지 627개를 휴대폰 로컬 참조로 수집했습니다. 화면에서 확인한 대상 selector 2·ordinal 147과 원문 한 줄의 해시가 양쪽에서 각각 한 번만 나타나 25줄 기준점 창이 유일하게 연결됐습니다. 다음 자동 단계는 이 단일 기준점의 상대 순서로 해당 원문 구간 전체를 대상 레코드에 투영해 사람 검토용 후보를 만들며, 원문·대상문·화자·순번은 계속 휴대폰 로컬에만 둡니다. 단일 기준점 투영은 승인된 원문 짝으로 간주하지 않습니다.
- 단일 기준점이 포함된 원문 구간 79줄을 대상 레코드 79개에 누락 없이 투영했습니다. 번역 준비 후보 36개, 글리프 복구 필요 38개, 구조 검토 필요 5개이며 화자 표지 68개와 나레이션 11개가 포함됩니다. 자동실행기는 원문·현재 한글·화자와 승인 칸을 담은 고정 ID 검토표를 `reports/HUMAN_REVIEW/`에 로컬 전용으로 만들고, 사용자가 작성하기 시작한 검토표를 다음 실행에서 덮어쓰지 않습니다. 이 연결은 사람이 승인하기 전까지 후보이며 `source_pairing_complete`, `speaker_assignment_complete`, `translation_build_eligible`은 계속 false입니다. 승인된 행만 `hancharacter`의 `translator_declared` 계약 입력으로 승격합니다.
