...
- DE=2·B=147에서 추정한 148번째 엔트리는 화면 경로에서 실제 read되지 않아 선택 대사로 승격하지 않습니다. 현재 화면 시험은 실제 read 0x44B1을 포함하는 유일한 23번째 엔트리의 200비트 경계를 사용하며, 픽셀을 그리지 않는 검증된 페이지 선택만 반복해 같은 `한다`를 정확히 200비트로 인코딩합니다. 이 경계의 확정 증거는 실패 상태로 계속 갱신되는 최신 캡처와 분리해 `analysis/evidence/v5_1_confirmed_group_capture.json`에 고정합니다.
