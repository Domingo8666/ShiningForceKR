...
- 확인된 대사 레코드의 18개 가시 심벌은 244개 글꼴 페이지 전체 조사에서 모두 유일한 글리프와 대응하지만, 런타임 초기 페이지는 9개 후보가 남아 있습니다. S25U 자동 단계는 첫 VDP 데이터 출력 시점의 슬롯 2 글꼴 뱅크를 관찰해 이 후보를 자동으로 좁힙니다. 원시 심벌·페이지 후보·레지스터 값은 휴대폰 로컬 보고서에만 두고, GitHub에는 후보 개수와 확인 여부만 게시합니다.
