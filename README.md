...
- 고유 후보 837개 중 빈 레코드 51개를 제외한 786개가 압축 비트 왕복에 성공했고, 625개가 단일 최적 텍스트 후보로 선별됐습니다. 실측 레코드도 동일 후보로 재확인됐습니다. 625개를 로컬 확장 대본으로 조립하되, 미해결 글리프 1,104회가 남아 있으므로 아직 번역·배포 가능 상태로 승격하지 않습니다.
- 확장 대본은 625개이며 현재 283개는 유니코드가 완전하고 342개에는 미해결 글리프가 있습니다. 확장 후보의 미일치 글리프를 실제 패치 글꼴 타일과 Galmuri7 기준으로 다시 비교해 고신뢰 자동 보정만 적용하고, 나머지는 비한글·오검출 후보로 분류합니다.
- 기존 카탈로그가 한글 11,172자만 비교해 문장부호·숫자·영문 등을 미해결로 남긴 경계를 수정했습니다. 검증된 Galmuri7의 8×8 수용 글리프 19,815개 중 비한글 가시 문자 8,546개를 추가로 정확 비교하되, 픽셀 거리가 0이고 후보가 하나뿐인 경우만 자동 복원합니다. 실제 대상에서는 13종·14회가 확정되어 번역 준비 레코드가 283개로 늘었고 미해결 발생은 1,095회로 줄었습니다. 후보가 겹치는 171종·981회와 미일치·범위 밖 항목은 추측하지 않고 보류합니다.
- 공개 영문 대본 25개 구간·1,409줄과 화자 표지 627개를 휴대폰 로컬 참조로 수집했습니다. 화면에서 확인한 대상 selector 2·ordinal 147과 원문 한 줄의 해시가 양쪽에서 각각 한 번만 나타나 25줄 기준점 창이 유일하게 연결됐습니다. 이 단일 기준점의 상대 순서로 해당 원문 구간 전체를 대상 레코드에 투영했으며, 원문·대상문·화자·순번은 계속 휴대폰 로컬에만 둡니다. 단일 기준점 투영은 승인된 원문 짝으로 간주하지 않습니다.
- 단일 기준점이 포함된 원문 구간 79줄을 대상 레코드 79개에 누락 없이 투영했습니다. 번역 준비 후보 36개, 글리프 복구 필요 38개, 구조 검토 필요 5개이며 화자 표지 68개와 나레이션 11개가 포함됩니다. 다음 자동 단계는 화자 전환과 대상 제어 신호의 전환·반복 관계를 휴대폰 로컬에서 대조하고 GitHub에는 고정 집계만 게시합니다. 기존 고정 ID 검토표는 기술자용 근거로만 유지하며 사용자가 원문-대상 연결에 `[x]`를 표시할 필요가 없습니다. 구조 상관이 확인돼도 후보 근거일 뿐이므로 `source_pairing_complete`, `speaker_assignment_complete`, `translation_build_eligible`은 계속 false이며, 실제 플레이 순서 증거와 사람의 한글 문장 승인을 거친 행만 `hancharacter`의 `translator_declared` 계약 입력으로 승격합니다.
