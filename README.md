...
- 확인된 대사 레코드의 18개 가시 심벌은 244개 글꼴 페이지 전체 조사에서 모두 유일한 글리프와 대응하지만, 런타임 초기 페이지는 9개 후보가 남아 있습니다. 첫 VDP 데이터 출력 시점의 슬롯 2 뱅크는 이 후보들의 글꼴 뱅크가 아니어서, 글꼴이 중간 RAM 버퍼를 거쳐 전송되는 경로로 판정했습니다. S25U 자동 단계는 이미 저장된 2프레임 원시 추적을 다시 재생하지 않고 오프라인 분석해 `ROM 글꼴 read → RAM write → VDP output` 연결을 찾습니다. 원시 심벌·페이지·뱅크·주소·opcode·레지스터 값은 휴대폰 로컬 보고서에만 두고, GitHub에는 후보 개수와 확인 여부만 게시합니다.
- 전체 대본이 추출되면 캐릭터별 원문/번역 행을 분리해 `hancharacter`의 `translator_declared` 계약으로 말투 손실·과잉 창작을 검사합니다. 현재 카탈로그는 canonical 0/10이므로 `precedent_backed` 통과를 꾸미지 않으며, 대본 추출 전에는 이 품질 게이트를 통과한 것으로 표시하지 않습니다.
...
- 확정 그룹 148개를 휴대폰 로컬에서 전부 열거했으며 선택된 148번째 레코드는 위 런타임 증거와 정확히 일치합니다. 기본 0xC9 초기 문맥으로는 77개가 비트 단위 왕복됐고, 51개 실제 Huffman 문맥 전수 조사에서는 유일한 최적 문맥이 118개를 왕복했습니다. 그룹 전체에 공통인 문맥은 없지만 기존 렌더러 추적의 첫 Huffman 벡터 접근이 실제 런타임 초기 문맥을 직접 가리키므로, 이 실측 문맥으로 확정되는 레코드를 먼저 글자 매핑하고 남는 레코드는 별도 형식으로 분류합니다. 문맥 값·압축 바이트·심벌은 휴대폰에만 둡니다.
- 실측 문맥으로 88개 레코드가 확정됐고 이 집합의 400개 가시 글리프 중 276개가 유일하게 매핑되어 초기 글꼴 페이지 후보가 9개에서 4개로 줄었습니다. 남은 60개는 휴대폰에 있는 깨끗한 원본 ROM의 같은 레코드와 로컬 비교해, 원문 압축 상태로 남은 엔트리와 별도 대상 형식 엔트리를 먼저 분리합니다. GitHub에는 동일·변경 레코드 개수만 게시합니다.
- 원본 대조 결과 148개 모두 패치에서 변경됐고 130개는 압축 길이도 달라, 남은 60개가 단순 원문 잔존이라는 가설은 제외했습니다. 각 레코드의 문맥별 해독 결과를 동일 심벌열로 합친 뒤, 유효한 페이지 선택 명령과 Galmuri7 글리프 일치율을 함께 점수화해 고유한 최적 대사 후보를 오프라인으로 선별합니다. 이 품질 추론 결과는 대본 후보 생성에만 쓰며 곧바로 재삽입 가능 상태로 승격하지 않습니다.
- 품질 선별로 120개 레코드의 고유한 최적 심벌열을 복구했고 7개는 동률, 21개는 유효 후보 없음으로 보존했습니다. 선택된 469개 글리프 중 404개는 정확히 매핑됐습니다. 나머지 65개는 원본 글꼴 타일과 Galmuri7 전체 11,172자 사이의 픽셀 거리를 계산하되, 유일 최단·2픽셀 이하·차순위와 2픽셀 이상 차이인 경우만 로컬 대본 후보의 고신뢰 보정으로 사용합니다.
- 65개 미해결 발생은 32개 서로 다른 글리프 좌표이며, 30개는 글꼴 범위 안이고 2개는 범위 밖입니다. 엄격한 거리·여유 조건을 모두 만족한 자동 보정은 0개라서 추측 문자를 넣지 않습니다. S25U 자동 단계는 120개 고유 대사 후보를 로컬 JSON/JSONL 코퍼스로 조립하고 미해결 글리프와 제어 코드를 명시적 표식으로 보존합니다. GitHub에는 문자·심벌·좌표·대사·화자 없이 집계와 해시만 게시하며, 원문 짝과 화자 ID를 확보하기 전에는 번역 빌드 가능 상태로 승격하지 않습니다.
- 원문-번역 대사 짝을 만들기 위해 깨끗한 원본 ROM의 같은 148개 레코드를 원본 Huffman 벡터 후보로 전수 왕복 검사합니다. 문맥과 원문 심벌은 휴대폰 로컬에만 두고, 각 레코드가 고유한 원문 심벌열을 갖는지의 집계만 게시합니다. 벡터가 원본 형식과 맞지 않으면 실패로 중단하지 않고 별도 탐색 필요 상태로 보존합니다.
- 영어 패치의 벡터 위치를 깨끗한 일본어 원본에 그대로 적용할 수 없음이 확인됐습니다. 원본 ROM 전체를 두 바이트 패리티별 슬라이딩 검사해 `0xFFFF 또는 0x4000..0x7FFF`인 256엔트리, 고유·증가 포인터 배열을 찾고 실제 트리 파싱까지 통과한 후보만 남깁니다. 고유 후보의 실제 위치·포인터·트리는 휴대폰 로컬에서만 원문 그룹 해독기로 전달합니다.
- 깨끗한 원본 전수 검사에서 구조 창은 79개였지만 실제 영어식 Huffman 트리로 파싱되는 벡터는 0개였습니다. 원문을 같은 코덱으로 억지 해독하지 않고, 원본 148개 길이 접두 레코드의 공통 종료 심벌과 내부 충돌 여부를 먼저 검사합니다. 구조가 일관되면 원문 심벌열과 120개 한국어 후보를 순번으로 로컬 짝짓기하고, 다음 단계에서 일본어 유니코드 문자표를 확정합니다.
- 같은 물리 위치의 원본 148개 레코드는 끝 심벌이 80종이고 본문 값도 255종이라 원문 대사가 아니라 패치가 덮어쓴 다른 원본 데이터로 판정했습니다. 120개 순번을 억지 원문과 연결하지 않습니다. 한국어판의 그룹 포인터 표에서는 유효 그룹 8개가 확인됐고, 저장된 호출 스택으로 상위 루틴 1개와 정적 호출 1개를 확인했지만 선택자는 런타임 계산값입니다. 호출부만 더 추적하기 전에 8개 포인터가 하나의 길이 접두 레코드 스트림 경계에 정렬되는지 검사해 전체 한국어 대사 추출 범위를 빠르게 확장합니다.
- 8개 포인터 중 4개만 같은 길이 접두 스트림 위상에 정렬되고 선택된 실측 레코드는 정렬됐습니다. 디코더의 엔트리 번호는 8비트이므로 각 포인터에서 접근 가능한 최대 256개 레코드를 뱅크 경계 안에서 열거한 뒤, 서로 겹치는 실제 레코드를 합쳐 전체 디코딩 후보 집합을 만듭니다.
- 주소 가능 범위 2,048개 슬롯 중 1,915개를 뱅크 경계 안에서 열거했고, 겹침을 제거하면 고유 레코드 후보는 837개입니다. 7개 선택자는 256개 전부에 도달했고 1개만 경계에서 중단됐으며, 실측 레코드는 정확히 재현됐습니다. 이 고유 후보를 기존 Huffman 왕복·글꼴 페이지 품질 필터에 일괄 통과시켜 실제 한국어 대사 후보만 선별합니다.
...
7개 명령에서는 출력이 없었고, 두 프레임 실측에서 확정 디코더 후보
실행 뒤 192회의 VDP 데이터 전송과 bank 0x21 주 렌더러의 1,636회
제어 전송이 순서대로 확인됐습니다.
...
한국어 렌더러의 제어 전송까지 같은 제한 프레임에서 이어진 경우만
소비 경로로 승인합니다.
이 결과로 원문 심벌과 실제 화면 글리프를
차례로 정렬하며, 정렬이 끝나기 전에는 배포용 번역 빌드를 허용하지
않습니다.

소비 경로가 확인되면 `tools/v5_1_visible_unicode_mapping.py`가 같은
가시 레코드의 로컬 심벌을 Galmuri7 글꼴 카탈로그와 대조합니다. 원시
심벌·코드포인트·문자·복원 문자열은 로컬 보고서에만 남고, GitHub에는
고유·모호·미일치 글리프 수와 다음 전체 스크립트 추출 게이트만
게시됩니다. 휴대폰에 로컬 카탈로그가 없으면 고정 커밋과 SHA-256으로
검증된 Galmuri7 BDF를 자동으로 받아 카탈로그를 만든 뒤 계속합니다.
