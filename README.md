...
- DE=2에서 포인터 0x43DE를 고르는 디코더 진입과 B=147을 확인했습니다. B를 148번째 문자열 번호로 본 0x49E0 시험과 0x44B1을 독립 스트림·23번째 엔트리로 본 시험은 모두 목표 `한다`를 표시하지 못해 기각했습니다. 다음 기술 시험은 패치 코드의 `INC B`와 런타임 read를 함께 사용해 B+1=148개 심벌의 고정 출력 블록을 무변경 왕복하고, 출력 위치를 바꾸지 않는 호환 엔트리 모두에 `한다` 표식을 넣어 한 번의 콜드 부팅으로 블록 가설을 판정합니다.
