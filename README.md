...
- DE=2·B=147에서 추정한 148번째 엔트리와 0x44B1을 포함하는 23번째 그룹 엔트리는 모두 시험 ROM의 같은 화면 경로에서 실제 read되지 않아 선택 대사로 승격하지 않습니다. 현재 화면 시험은 런타임에서 직접 읽혔고 픽셀 변화까지 확인된 0x44B1 스트림의 87비트 경계를 사용합니다. 픽셀을 그리지 않는 검증된 페이지 선택만 반복해 `한다`를 정확히 87비트 `EA4A95D4A95D539D512D10`으로 인코딩하고 마지막 이웃 비트도 보존합니다. 그룹 분석은 음성 증거로 `analysis/evidence/v5_1_confirmed_group_capture.json`에 고정합니다.
