...
- 확정 그룹 148개를 휴대폰 로컬에서 전부 열거했으며 선택된 148번째 레코드는 위 런타임 증거와 정확히 일치합니다. 기본 0xC9 초기 문맥으로 77개가 비트 단위 왕복되고 71개가 남았으므로, 자동 단계는 51개 실제 Huffman 문맥을 모든 레코드에 대입해 그룹 전체에 공통인 단 하나의 초기 문맥이 있는지 먼저 판별합니다. 문맥 값·압축 바이트·심벌은 휴대폰에만 두며, 이 가설이 틀리면 레코드 변형 분류로 자동 전환합니다.
