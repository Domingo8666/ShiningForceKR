...
- 같은 물리 위치의 원본 148개 레코드는 끝 심벌이 80종이고 본문 값도 255종이라 원문 대사가 아니라 패치가 덮어쓴 다른 원본 데이터로 판정했습니다. 120개 순번을 억지 원문과 연결하지 않습니다. 한국어판의 0x3FE8 그룹 포인터 표와 0x33FA 디코더 호출 형태를 전수 검사해 실제 사용 선택자와 최대 엔트리 후보를 로컬에서 수집하고, 전체 한국어 대사 그룹 추출 범위를 확장합니다.
