...
- 같은 물리 위치의 원본 148개 레코드는 끝 심벌이 80종이고 본문 값도 255종이라 원문 대사가 아니라 패치가 덮어쓴 다른 원본 데이터로 판정했습니다. 120개 순번을 억지 원문과 연결하지 않습니다. 한국어판의 그룹 포인터 표에서는 유효 그룹 8개가 확인됐지만 디코더 직접 호출은 없었습니다. 이미 저장된 휴대폰 로컬 호출 스택으로 상위 루틴을 찾고 그 호출부의 선택자·엔트리 후보를 집계해 전체 한국어 대사 그룹 추출 범위를 확장합니다.
