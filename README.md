...
bank 8·물리 0x0203DE에서 다시 얻은 5개 화면은 초상·대화창·주변
한국어와 입력 뒤 화면 전이를 유지했지만 시험 문구 `한다`를 표시하지
않았습니다. 이 결과는 성공이 아니라 사람 검토 실패로 기록했습니다.
자동실행은 해당 캡처·빌드 해시와 실패 판정이 일치할 때만 0x0203DE를
제외하고 다음 실제 디코더 스트림 0x0203E8을 시험합니다. 캡처 픽셀,
