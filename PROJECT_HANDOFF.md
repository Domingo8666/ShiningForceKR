...
14. 완료: 정확한 0x4913 레코드 선택 뒤 가장 바깥 호출 복귀까지의 제한 trace는 7개 명령과 VDP 출력 0회를 확인했습니다. 첫 프레임 제한 trace에서는 12,371개 엔트리, 한국어 렌더러 진입 1회, 렌더러 내부 VDP 제어 출력 800회와 프레임 데이터 출력 192회를 확인했지만 데이터 192회는 렌더러 진입 전이었습니다. 다음 자동 실행은 두 프레임만 추적하고, 렌더러 제어 출력 뒤의 VDP 데이터 출력만 순서가 이어진 소비 경로로 승인합니다.
