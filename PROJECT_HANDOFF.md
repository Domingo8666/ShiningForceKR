...
14. 완료: 정확한 0x4913 레코드 선택 뒤 가장 바깥 호출 복귀까지의 제한 trace는 7개 명령과 VDP 출력 0회를 확인했습니다. 다음 자동 실행은 같은 앵커에서 첫 프레임 경계까지만 trace를 확장하고, 원시 값은 로컬에만 보존한 채 VDP 출력 집계만 게시합니다.
