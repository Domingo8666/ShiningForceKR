...
60. 0x43DE부터 148개 엔트리를 바이트 패딩 없이 연속 디코드·재인코드한 결과는 정확히 일치했지만, 148번째 엔트리의 바이트 범위는 0x49E0..0x49EE로 목표 read 0x44B1을 포함하지 않았습니다. 따라서 B=147의 엔트리 서수 해석을 폐기하고, 같은 검증된 prefix 안에서 0x44B1과 겹치는 실제 엔트리 후보를 별도로 계산하도록 schema v5를 추가했습니다.
...
11. 자동 재검증 중: 실제 디코더 stream 0x0203DE·0x0203E8·0x020447·0x020462·0x020473은 문구 미표시로 실패했습니다. 0x0204B1은 최대 459픽셀 변화가 있었지만 당시 `한` 글리프 매핑이 틀렸으므로 사람 판정을 승격하지 않습니다. 교정한 31비트 `한다`로 같은 확정 엔트리를 다시 빌드·캡처합니다.
