...
11. 자동 재검증 중: B=147의 148번째 엔트리, 0x44B1 포함 23번째 그룹 엔트리, 0x44B1 독립 스트림 해석은 모두 목표 `한다`를 표시하지 못해 제외했습니다. 패치 디코더 진입부의 `INC B`를 B+1=148개 심벌의 고정 출력 길이 가설로 분리하고, 0x43DE부터 무변경 왕복과 0x44B1 포함을 먼저 검증한 뒤 출력 오프셋을 유지하는 모든 호환 엔트리에 표식을 넣어 한 번의 콜드 부팅으로 판정합니다.
