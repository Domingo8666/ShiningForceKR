...
1. S25U 원클릭 파이프라인으로 v5 전수 디코드 집계를 생성하고 안전 관측 요약을 게시합니다.
2. 전수 집계로 정렬 우선순위를 고정하되, 정적 결과는 실행 소비 증거로 승격하지 않습니다.
3. 정렬 클러스터 통합 범위의 slot 0/1/2 read breakpoint 중 실제 히트와 매퍼 상태를 기록합니다.
4. 히트 PC와 0x87000 런타임의 호출·데이터 흐름을 연결해 조회표를 확정합니다.
