...
4. 검증된 한국어 Huffman 벡터 물리 0x080100..0x0802FF가 slot 1 또는 2에 bank 0x20으로 매핑된 실제 read를 한 콜드 부팅에서 동시에 감시합니다. read가 없으면 5개 단계 화면의 해시로 자동 입력이 안정된 화면에서 멈췄는지 판정하고, S25U 로컬의 `05_confirm-16.png` 한 장으로 이름 입력 화면 여부를 확인해 END 이동 경로를 교정합니다.
