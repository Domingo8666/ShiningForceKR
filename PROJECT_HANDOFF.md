...
11. 자동 재검증 중: B=147의 148번째 엔트리와 0x44B1 포함 23번째 그룹 엔트리는 목표 read가 재현되지 않아 표시 대사 후보에서 제외했습니다. 실제 런타임 시작점 0x0204B1의 87비트 스트림을 길이와 마지막 이웃 비트를 보존한 `한다`로 교체해 같은 콜드 부팅 경로를 다시 캡처합니다.
